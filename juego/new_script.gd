...
func _coroutine_show_temporal(num: int, segundos: float, siguiente: int = -1) -> void:
	...
	# al terminar oculta el globo actual
	...
	if siguiente >= 1:
		# evitar duplicados si ya fue mostrado
		if not _globo_triggered.get(siguiente, false):
			_coroutines_running[siguiente] = true
			_coroutine_show_temporal(siguiente, 0, -1)  # 0 seg -> aparecerá y no esperar
...
func _procesar_globos() -> void:
	...
	# Globo9: aparece cuando avena desaparece (y estamos en fase donde debería)
	if not _globo_triggered[9] and not avena.visible and not cuchara_madera.visible and fase == 5:
	...
	# Globo16: cuando aparece tabla, banano con cascara y cuchillo
	# se muestra 7s y luego globo17 (por corutina)
	...
	# Globo17: será mostrado por la corutina de 16 -> 17
		...
		# ----------------
			...
			if dragging_node:
				...
				if dragging_node == banano_sin_cascara and fase == 7:
					if banano_sin_cascara.get_rect().intersects(tabla.get_rect()):
						# colocarlo centrado en tabla (usa tabla.position)
						banano_sin_cascara.position = tabla.position
					...
					if dragging_node.get_rect().intersects(plato_con_avena.get_rect()):
						if dragging_node.visible:
							...
							if ingredientes_agregados >= 3:
								...
								# restaurar posicion original de avena_final
